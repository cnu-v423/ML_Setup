...
class EnsemblePredictor:
    ...
    def merge_augmented_predictions(self, predictions):
        ...
        # print("function :: merge_augmented_predictions")
    ...
    def predict(self, image_path, output_path):
        """Run ensemble prediction with all models - handles any file size"""
        ...
        for name, model in self.models.items():
            ...
            batch_size = self.config.get('data', {}).get('batch_size', 32)  # Default batch size
            ...
            for chunk_start in range(0, total_tiles, batch_size):
                ...
                # Process tiles in this chunk based on model type
                ...
                for tile in chunk_tiles:
                    try:
                        if name == "building":
                            ...
                            # Apply Sobel filter for water bodies
                ...
                if processed_chunk_tiles:
                    ...
                    for i, tile in enumerate(chunk_array):
                        try:
                            ...
                            print(f"Shape of augmented_tiles for tile {chunk_start + i}: {augmented_tiles.shape}") # ADD THIS LINE

                            num_patches = augmented_tiles.shape[0]
                            aug_predictions = []
                            
                            # Process augmented tiles in smaller sub-batches with Triton
                            aug_batch_size = min(self.max_batch_size, num_patches)
                            for aug_start in range(0, num_patches, aug_batch_size):
                                aug_end = min(aug_start + aug_batch_size, num_patches)
                                batch_patches = augmented_tiles[aug_start:aug_end]
                                ...
                                try:
                                    ...
                                    print(f"Inference failed for batch {aug_start // aug_batch_size + 1}: {e}")
                                    continue
            ...
            #     output_path,
            #     f"{os.path.splitext(os.path.basename(output_path))[0]}_{name}_res{str(getattr(self, 'target_resolution', self.tile_size)).replace('.', 'p')}m.tif"
            # )
            ...
            with rasterio.open(
                ...
                dst.update_tags(
                    ...
                    TARGET_RESOLUTION=str(getattr(self, 'target_resolution', self.tile_size)),
                    ZOOM_APPLIED="TRUE"
                )
            ...
            #     output_path,
            #     f"{os.path.splitext(os.path.basename(output_path))[0]}_{name}_res{str(getattr(self, 'target_resolution', self.tile_size)).replace('.', 'p')}m_prob.tif"
            # )
        ...
        final_output = os.path.join(
            output_path,
            f"{os.path.splitext(os.path.basename(output_path))[0]}_multiclass_res{str(getattr(self, 'target_resolution', self.tile_size)).replace('.', 'p')}m.tif"
        )
        ...
        stats = {
            ...
            'target_resolution': getattr(self, 'target_resolution', self.tile_size)
        }
    ...
    #         individual_output = os.path.join(
    #             output_path,
    #             f"{os.path.splitext(os.path.basename(output_path))[0]}_{name}_{self.tile_size}.tif"
    #         )
    ...
    #         prob_output = os.path.join(
    #             output_path,
    #             f"{os.path.splitext(os.path.basename(output_path))[0]}_{name}_{self.tile_size}_prob.tif"
    #         )
    #         with rasterio.open(
    #             prob_output,
    #             'w',
