...
def create_vegetation_tiles(input_tif, input_shp, output_dir, tile_size=512, overlap_percent=0.25):
    ...
    tiles_dir = os.path.join(output_dir, f"tiles_{tile_size}")
    masks_dir = os.path.join(output_dir, f"masks_{tile_size}")
...
def main():
    ...
    parser.add_argument('--input_tif', required=True, help='Path to input RGB TIFF file')
    parser.add_argument('--input_shp', required=True, help='Path to vegetation mask shapefile')
    parser.add_argument('--output_dir', required=True, help='Output directory for tiles and masks')
    parser.add_argument('--tile_size', type=int, default=512, help='Tile size in pixels (default 512)')
    parser.add_argument('--overlap', type=float, default=0.25, help='Tile overlap as fraction (default 0.25 = 25%%)')
