...
def create_unet_model(config, **kwargs):
    """Placeholder for create_unet_model"""
    # return build_unet_resnet50(*args, **kwargs)
    input_size = config["data"].get("input_size", 256)
    
    # Detect number of classes from class_weights length
    num_classes = 1
    
    use_adapter = config.get("model", {}).get("use_adapter", False)
    freeze_backbone = config.get("model", {}).get("freeze_backbone", True)

    return build_unet_resnet50(
